...
export class SakuraScriptRouting implements EventRouting {
...
export class SakuraScriptPlayer {
  sakurascriptExecuter: SakuraScriptExecuter;
  shiorif: Shiorif;

  constructor(sakurascriptExecuter: SakuraScriptExecuter, shiorif: Shiorif) {
...
export class SakuraScriptController extends GhostKernelController {
  async start() {
    ...
    this.kernel.registerComponent(new SakuraScriptPlayer(sakurascriptExecuter, shiorif));
  }

  halt() {
    this.kernel.component(SakuraScriptExecuter).abortExecute();
  ...
  _handle_balloon(token: SakuraScriptToken) {
    ...
    const surface = scope.surface();
    const blimp = scope.blimp();
    const shellState = <any> this.kernel.component(ShellState);
